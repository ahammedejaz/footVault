...
insert into public.homepage_sections (section_type, title, subtitle, payload, sort_order) values
  ...
  ('rich_text', 'How sizing works here', NULL, '{"body":"Every size on a product page is a size we physically hold. If it is listed, it is on the shelf — we do not show a size run and then cancel your order.\n\n- Sizes are UK, the way Indian shops quote them\n- **Delivery is free over {{free_shipping_threshold}}**\n- Damaged on arrival? Tell us within {{return_window}}"}'::jsonb, 7);
