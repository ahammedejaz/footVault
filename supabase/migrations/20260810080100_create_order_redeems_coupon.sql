-- `create_order_with_stock` learns to redeem a coupon, atomically (§9F).
--
-- One new parameter, `p_coupon_code`. Everything the code promises happens in
-- the same transaction as the stock decrement: the coupon row is locked, every
-- rule is re-checked against the subtotal this function just computed under
-- that lock, the discount is recomputed from the coupon's own terms, the
-- redemption is written to the ledger, and the counter moves. The preview the
-- customer saw in the bag was advisory; this is the authority.
--
-- ## Why `for update` on the coupon row is the whole feature
--
-- A usage limit enforced by read-then-write passes every single-threaded test
-- and oversells in production. Two checkouts racing on the last use of a code
-- serialise on this lock: the second reads the count the first just wrote and
-- is refused. `scripts/audit/coupons.ts` proves exactly this, concurrently,
-- not sequentially.
--
-- ## No stacking (owner's decision, 2026-08-10)
--
-- A coupon and the prepaid discount do not combine. The checkout offers the
-- customer the larger of the two and names which applied; when a code is
-- passed here, the coupon *is* the discount and the prepaid part is zero. The
-- alternative — both, summed — was the plan's recommendation and was
-- overruled: one discount with one name is the rule that fits on a receipt.
--
-- With no code passed, the only discount with a source is the prepaid one, so
-- `discount_total` is written equal to it. Any excess in `p_discount_total`
-- beyond the prepaid part names no mechanism, and writing it would violate
-- `orders_discount_parts_sum` — it is dropped rather than laundered.
--
-- ## What a refusal tells the customer (owner's decision, 2026-08-10)
--
-- Errcode `CPNRJ`, message = one keyword. `expired`, `minimum`, `limit` and
-- `used` are actionable and safe to say. Everything else — no such code, not
-- for this customer, disabled, not started yet — is one `unknown`, so a code
-- can be neither enumerated nor confirmed early. Audience is checked *before*
-- expiry so a not-for-you code reads as unknown even when it is also expired.
--
-- ## Rounding
--
-- The shop's one rule, from `src/lib/payments/discount.ts`: round UP to a
-- whole rupee in the customer's favour, then cap — `max_discount` and the
-- goods total, in that order, cap after rounding so a figure sitting on its
-- ceiling is not pushed through it. `ceil(a/b) = (a+b-1)/b` in bigint terms.
--
-- ## Guests
--
-- `audience = 'specific_customers'` refuses guests (there is no customer to
-- match). `per_user_limit` cannot be counted for a guest and does not refuse
-- one: the honest guest would be punished for the shop's inability to count,
-- and the global `usage_limit` still bounds the total spend.
...
begin
  ...
  if p_coupon_code is not null then
    ...
    -- No stacking: the coupon is the discount.
    v_discount := v_coupon_discount;
    v_prepaid  := 0;
    v_coupon_id := v_coupon.id;
    v_coupon_code := v_coupon.code;
  else
    -- No coupon: the prepaid incentive is the only discount with a source.
    -- Clamped exactly as before, then discount_total is written equal to it so
    -- orders_discount_parts_sum holds by construction.
    v_discount := least(greatest(coalesce(p_discount_total, 0), 0), v_subtotal);
    v_prepaid := least(greatest(coalesce(p_prepaid_discount, 0), 0), v_discount);
    v_discount := v_prepaid;
  end if;
...
revoke all on function public.create_order_with_stock(
  uuid, jsonb, text, public.order_status, public.payment_status, bigint, bigint,
  uuid, text, text, text, text, bigint, bigint, bigint, bigint, text, integer,
  bigint, bigint, bigint, text, text, text) from public, anon, authenticated;

grant execute on function public.create_order_with_stock(
  uuid, jsonb, text, public.order_status, public.payment_status, bigint, bigint,
  uuid, text, text, text, text, bigint, bigint, bigint, bigint, text, integer,
  bigint, bigint, bigint, text, text, text) to service_role;
