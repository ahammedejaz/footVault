-- `create_order_with_stock` learns which part of the discount was for paying
-- online.
--
-- One new parameter, one new column on the insert, one new clamp. Everything
-- else is the body from 20260809110300 unchanged — restated in full because
-- Postgres cannot add a parameter to a function without recreating it, and a
-- partial rewrite here would be a silent behaviour change in the one function
-- that moves stock.
--
-- ## Why the split is recomputed here rather than trusted
--
-- `p_discount_total` is already clamped to the subtotal under the row lock, for
-- the reason the Phase 8 comment gives: the function re-reads prices, so a
-- discount applied only in TypeScript could write a grand total higher than the
-- one the customer was shown. The prepaid part gets the identical treatment and
-- is then clamped to the clamped total, so `prepaid_discount <= discount_total`
-- holds by construction rather than by the caller having been careful. The CHECK
-- added by the sibling migration would otherwise be reachable from the checkout
-- with a bad payload — and a constraint violation there is a customer seeing
-- "something went wrong" after their stock was claimed.
--
...
begin
  ...
  -- The part of the discount that was given for paying online, held inside the
  -- whole it is part of. See the header: this is what makes the CHECK
  -- unreachable from the checkout rather than merely unlikely.
  ...
  v_subtotal, v_discount, v_prepaid, v_shipping, 0, v_total,
...
revoke all on function public.create_order_with_stock(
  ...
  bigint, bigint, bigint, text, text) from public, anon, authenticated;

grant execute on function public.create_order_with_stock(
  uuid, jsonb, text, public.order_status, public.payment_status, bigint, bigint,
  uuid, text, text, text, text, bigint, bigint, bigint, bigint, text, integer,
  bigint, bigint, bigint, text, text) to service_role;
