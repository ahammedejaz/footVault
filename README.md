...
| `npm run images:reprocess`        | Rebuild every catalogue photograph from `originals/` under the current `PIPELINE_VERSION` and repoint the rows — `--dry-run` to preview, `--stage` for staging. New paths per version, so a half-finished run never half-breaks the shop |
...
| `npm run audit:site-images`       | Every picture the owner can change: `frameRect` never leaves the photograph and keeps the frame's shape; every frame preset renders to its declared size and format (a corner framing must come back the corner's colour, a logo's corner must be transparent); the Choose/Adjust controls are on screen and touch-sized at 390px on all four surfaces; and one real upload end to end — signed URL, Storage, render, row, address on screen |
...
| `npm run audit:deploy-drift`      | Production serves the tip of `origin/main` — `GET /api/version` against local git; exits non-zero when either side is unknowable. Opens with `verdictLogic()`: the health card's verdict held against a mocked GitHub (404/403/drift/garbage/network), so no degraded branch can read as "in sync" |
