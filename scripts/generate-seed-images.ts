...
const PROFILES: Record<Shape, Profile> = {
  sneaker: {
    ...
    sole: "M10 236 L754 234 C762 274 742 300 700 302 L66 302 C22 300 2 274 10 236 Z",
  ...
  sports: {
    ...
    sole: "M6 220 L758 216 C772 268 746 302 692 306 L70 306 C16 302 -6 266 6 220 Z",
    detail:
  ...
  formal: {
    ...
    sole: "M26 250 L724 246 L722 278 C716 285 704 288 688 288 L64 288 C34 286 18 268 26 250 Z M572 276 L688 272 C698 292 694 308 680 313 C674 315 666 316 656 316 L598 316 C586 316 578 306 578 294 Z",
    // Brogue seam over the vamp, and the facing where the lacing sits.
    detail:
      "M358 130 C408 142 478 148 566 146 M462 86 L494 136 M544 78 L512 136",
    outsoleFrom: 0.55,
  },
  boot: {
    upper:
    ...
    sole: "M14 242 L730 240 C744 284 720 308 674 310 L70 310 C22 308 2 284 14 242 Z",
  ...
  sandal: {
    ...
    sole: "M34 242 L722 238 C734 280 710 306 660 308 L96 308 C44 304 22 280 34 242 Z",
    straps:
  ...
  slide: {
    sole: "M42 238 L716 234 C730 278 704 306 652 308 L106 308 C52 304 30 280 42 238 Z",
    // One wide band across the forefoot is the whole design.
    straps:
      "M156 236 C160 190 200 166 268 160 L520 160 C594 166 646 192 660 232",
    strapWidth: 44,
    detail: "M48 272 L710 268",
    outsoleFrom: 0.58,
  },
  flipflop: {
    sole: "M52 238 L706 234 C720 278 694 306 642 308 L116 308 C62 304 40 280 52 238 Z",
...
function main() {
  ...
  for (const product of products) {
    for (const color of product.colors) {
      ...
      writeFileSync(
        join(OUT_DIR, `${stem}-hero.svg`),
        heroProfileSvg(product, color),
      );
  ...
  for (const category of categories) {
    ...
    if (!art) continue;
    writeFileSync(
      join(OUT_DIR, `category-${category.slug}.svg`),
      categorySvg(art.shape, art.hex),
    );
    count += 1;
  }
