...
*/
assertNotProduction("run audit:bag");
...
async function addFirstAvailableSize(
  page: Page,
  slug: string,
): Promise<string | null> {
...
async function main() {
  ...
  await assertServerNotProduction(BASE, "run audit:bag");
  ...
  check(
    "three items added from three product pages",
    added.length === 3,
    added.join(", "),
  );
  ...
  check(
    "the header badge counts them",
    (await bagCount(page)) === 3,
    `badge = ${await bagCount(page)}`,
  );
  ...
  check(
    "the guest token is an httpOnly cookie",
    Boolean(guest?.httpOnly),
    guest ? `httpOnly=${guest.httpOnly}` : "absent",
  );
  check(
    "the bag is not in localStorage",
    !(
  ...
  check(
    "the bag survived closing the browser",
    (await bagCount(back)) === 3,
  ...
  const rows = drawer
    .locator("li")
    .filter({ has: back.locator("button[aria-label^='One more']") });
  await rows.first().waitFor({ timeout: 10_000 });
  check(
    "the drawer lists all three lines",
    (await rows.count()) === 3,
  ...
  */
  ...
  const heading = await back
    .getByRole("heading", { level: 1 })
    .first()
    .textContent();
  check(
    "the cart page renders",
    (heading ?? "").toLowerCase().includes("your bag"),
    heading ?? "",
  );
  check(
  ...
  */
  ...
  check(
    ...
    (await back.getByRole("link", { name: /checkout/i }).count()) > 0,
  );
  ...
  check(
    "removing offers an undo",
    (await back.getByRole("button", { name: "Undo" }).count()) > 0,
  );
