...
export default async function ProductOpengraphImage({
  ...
  const product = await prerenderOrDefer(`og image(${slug})`, () =>
    getProduct(slug),
  );
  ...
  const price = product
    ? formatPaise(product.salePrice ?? product.basePrice)
    : "";
  ...
  return new ImageResponse(
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        {price ? (
          <div style={{ fontSize: 42, color: "#fe9301" }}>{price}</div>
        ) : null}
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div
          style={{ display: "flex", gap: 18, fontSize: 34, letterSpacing: 3 }}
        >
          {sizes.map((entry) => (
            <div
              key={entry.size}
